...
class CompressionPluginRegistryTest : public ::testing::Test {
protected:
...
TEST_F(CompressionPluginRegistryTest, factory_mutex) {
  CompressionPluginRegistry &instance = CompressionPluginRegistry::instance();
...
TEST_F(CompressionPluginRegistryTest, all)
{
  CompressionProfile profile;
  ...
  CompressionPlugin *plugin = 0;
  {
    ...
    EXPECT_EQ(-EEXIST, instance.load("zlib", directory, &plugin, &cerr));
    EXPECT_EQ(-ENOENT, instance.remove("does not exist"));
    EXPECT_EQ(0, instance.remove("zlib"));
    EXPECT_EQ(0, instance.load("zlib", directory, &plugin, &cerr));
  }
}
